    ...
    : ControllerBase
{
    ...
    public async Task<IActionResult> CreateOrUpdateAsync(
        ...
        ExamQuestionUpdateDto examUpdateDto,
        CancellationToken cancellationToken = default
    )
    {
        logger.LogInformation("CreateOrUpdateAsync (Question) called with examId: {ExamId}, topicId: {TopicId}, questionId: {QuestionId}", examId, topicId, questionId);

        if (!User.ResolveOrganizationId(out var organizationId))
        {
            logger.LogWarning("Unauthorized access attempt to create/update question {QuestionId} - no organization ID found", questionId);
            return Unauthorized();
        }
        ...
        {
            logger.LogWarning("Topic {TopicId} under exam {ExamId} not found for question {QuestionId}", topicId, examId, questionId);
            return Unauthorized();
        }
        if (topic.OrganizationId != organizationId)
        {
            logger.LogWarning("Unauthorized: Organization {OrganizationId} tried to modify question {QuestionId} under topic {TopicId}", organizationId, questionId, topicId);
            return Unauthorized();
        }
        ...
        {
            logger.LogWarning("Unauthorized: Organization {OrganizationId} tried to modify question {QuestionId} belonging to another org {OtherOrg}", organizationId, questionId, existing.OrganizationId);
            return Unauthorized();
        }
        ...
        else
        {
            ...
            // No fields in ExamQuestionUpdateDto to update currently.
        }
    ...
    {
        logger.LogInformation("GetAllAsync (Questions) called with examId: {ExamId}, topicId: {TopicId}, skip: {Skip}, take: {Take}", examId, topicId, skip, take);

        if (!User.ResolveOrganizationId(out var organizationId))
        {
            logger.LogWarning("Unauthorized access attempt to list questions for topic {TopicId} - no organization ID found", topicId);
            return Unauthorized();
        }
        ...
        logger.LogDebug("Querying questions for org {OrganizationId}, exam {ExamId}, topic {TopicId} with pagination skip={Skip}, take={Take}", organizationId, examId, topicId, skip, take);
        var query = examDbContext.ExamQuestions
            .Where(q => q.OrganizationId == organizationId && q.ExamTopic!.Identifier == topicId && q.ExamTopic!.Exam!.Identifier == examId)
            .OrderBy(q => q.Title);
        ...
        return Ok(data.Select(_ => new ExamQuestionListingDto()).ToArray());
    }
    ...
    {
        logger.LogInformation("GetAllCountAsync (Questions) called with examId: {ExamId}, topicId: {TopicId}", examId, topicId);

        if (!User.ResolveOrganizationId(out var organizationId))
        {
            logger.LogWarning("Unauthorized access attempt to get question count for topic {TopicId} - no organization ID found", topicId);
            return Unauthorized();
        }

        logger.LogDebug("Counting questions for org {OrganizationId}, exam {ExamId}, topic {TopicId}", organizationId, examId, topicId);
    ...
    {
        logger.LogInformation("DeleteAsync (Question) called with examId: {ExamId}, topicId: {TopicId}, questionId: {QuestionId}", examId, topicId, questionId);

        if (!User.ResolveOrganizationId(out var organizationId))
        {
            logger.LogWarning("Unauthorized access attempt to delete question {QuestionId} - no organization ID found", questionId);
            return Unauthorized();
        }
        ...
        {
            logger.LogWarning("Unauthorized deletion attempt: Org {OrganizationId} tried to delete question {QuestionId} belonging to org {QuestionOrg}", organizationId, questionId, existing.OrganizationId);
            return NoContent();
        }
        if (existing.ExamTopic?.Identifier != topicId)
        {
