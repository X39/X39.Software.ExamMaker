...
namespace X39.Software.ExamMaker.WebApp.Pages.Management.Exams.Arg;
    ...
    public sealed class ExamViewModel(ExamListingDto exam, Func<Task> stateHasChanged, IExamRepository examRepository, IExamTopicRepository examTopicRepository, IExamQuestionRepository examQuestionRepository, IExamAnswerRepository examAnswerRepository)
    {
                ...
                Task.Run(async () =>
                    {
                        ...
                        await examRepository.UpdateExamAsync(Identifier, value, null);
                        exam.Title = value;
        ...
        public string Preamble
        {
            ...
            set
            {
                Task.Run(async () =>
                    {
                        using var busy = BusyHelper.Busy();
                        await examRepository.UpdateExamAsync(Identifier, null, value);
                        exam.Preamble = value;
                        await stateHasChanged();
                    }
                );
        ...
        public async Task InitializeAsync()
        {
            ...
            for (var topicIndex = 0; topicIndex < topicCount; topicIndex += takeAmount)
            {
                ...
                {
                    var viewModel = new TopicViewModel(Identifier, topic, stateHasChanged, examTopicRepository, examQuestionRepository, examAnswerRepository);
